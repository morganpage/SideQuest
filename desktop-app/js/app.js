class App {
  constructor() {
    ...
    this.sync_selected_button.addEventListener('click', () =>
      this.openSyncSelected()
    );
    this.patch_make_data_backup = document.querySelector(
      '.patch-make-data-backup'
    );
    this.title = document.querySelector('.header-title');
    this.searchFilterContainer = document.querySelector(
      ...
      if (this.open_url_custom) {
        ...
        switch (url[0]) {
          ...
          case 'sidequest://sideload/':
            ...
            this.setup.installApk(url[1]).then(() => {
          ...
          default:
            this.showStatus(
              'Custom protocol not recognised: ' + (data || ''),
              true,
              true
            );
            break;
    ...
    ipcRenderer.on('info', (event, data) => {
      try {
        ...
        if (data.beatsaber) {
          if (this.bsaber) {
            this.spinner_loading_message.innerText =
    ...
    this.mkdir(appData)
      ...
      .then(async () => {
        if (!fs.existsSync(path.join(appData, 'sources.txt'))) {
          fs.createReadStream(path.join(__dirname, 'sources.txt')).pipe(
            fs.createWriteStream(path.join(appData, 'sources.txt'))
          );
        }
        this.current_data = [];
        ...
        this.toggleLoader(true);
        this.spinner_loading_message.innerText =
          'Downloading 1, This might take some time on first launch please wait...';
        ...
        this.spinner_loading_message.innerText =
          'Downloading 2, This might take some time on first launch please wait...';
        this.bsaber.downloadQuestSaberPatch();
        this.bsaber.setBeatVersion().catch(e => {});
      });
  }
  async mkdir(path) {
  ...
  setupMenu() {
    document
      .querySelector('.reset-new-backup')
      .addEventListener('click', () => {
        this.backupsModal.close();
        this.resetBase();
      });
    document
      .querySelector('.backup-new-master-modal')
      .addEventListener('click', () => {
        ...
        files => {
          ...
          if (files !== undefined && files.length === 1) {
            ...
            if (this.setup.doesFileExist(current_path)) {
              fs.copyFileSync(
                current_path,
                path.join(appData, 'bsaber-backups', 'bsaber-base-old.apk')
              );
            }
            fs.copyFileSync(files[0], current_path);
            fs.copyFileSync(
              files[0],
              path.join(appData, 'bsaber-backups', 'bsaber-base.apk')
            );
    ...
    this.patch_make_data_backup.addEventListener('click', () => {
      ...
      this.bsaber.backUpBeatSaberBinary().catch(e => {
        this.showStatus(e.toString(), true, true);
      });
    });
    ...
    this.patch_beatsaber.addEventListener('click', () => {
      ...
      let questSaberPatchContainer = document.getElementById(
      ...
      if (this.patch_beatsaber.style.display !== 'none' && songs.length) {
          ...
          this.bsaber
            ...
            .then(result => {
              ...
              let skipped = Object.keys(
                result && result.installSkipped ? result.installSkipped : {}
              ).map(d => {
                return (
                  '&nbsp;&nbsp;<b>' + d + '</b>:' + result.installSkipped[d]
                );
              });
              questSaberPatchContainer.innerHTML =
            ...
            .catch(e => {
              ...
              questSaberPatchContainer
                .querySelector('.install-base-reset')
                .addEventListener('click', () => {
                  this.patchModal.close();
                  this.spinner_loading_message.innerText =
                    'Installing Base APK';
                    ...
                    .then(() => {
                      if (!this.bsaber.resetPatched()) {
                        return Promise.reject(
                          new Error(
                            'Failed to reset the patched base, the base backup does not exist.'
                          )
                        );
                      }
                    })
        ...
        this.patchModal.close();
        this.showStatus(
          'Something went wrong, do you have any songs selected?',
          true,
          true
        );
      }
    ...
    document
      .querySelector('.open-data-backup-folder')
      .addEventListener('click', () => {
        shell.openItem(path.join(appData, 'bsaber-data-backups'));
      });
    document
      .querySelector('.open-songs-main-one')
      .addEventListener('click', () => {
    ...
    document
      ...
      .addEventListener('click', () => {
        ...
        this.bsaber.deleteFolderRecursive(
          path.join(appData, 'saber-quest-patch')
        );
    ...
    document
      .querySelector('.make-data-backup')
      .addEventListener('click', () => {
        this.backupsModal.close();
        this.toggleLoader(true);
        ...
        this.bsaber
          ...
          .then(() => {
            this.toggleLoader(false);
            this.showStatus(
              'Beat player data backed up to ' +
                path.join(appData, 'bsaber-data-backups')
            );
          })
          .catch(e => {
            this.toggleLoader(false);
    ...
    this.sync_songs_now.addEventListener('click', () =>
      this.openCustomLevels()
    );
    document
      .getElementById('musiclist')
      .addEventListener('click', () => this.openCustomLevels());
    ...
    this.enable_wifi.addEventListener('click', () =>
      this.setup.enableWifiMode()
    );
    document
      .getElementById('close-app')
      .addEventListener('click', () => remote.getCurrentWindow().close());
    document
      .getElementById('open-debugger')
      .addEventListener('click', () =>
        remote.getCurrentWindow().toggleDevTools()
      );
    document
      .getElementById('installed-apps')
      .addEventListener('click', () => this.openPackageList());
    document
      .getElementById('device-settings')
      .addEventListener('click', () => this.openDeviceSettings());
    document
      .querySelector('.confirm-uninstall-app')
      .addEventListener('click', () => {
        if (this.current_uninstall_package) {
          this.setup
            .uninstallApk(this.current_uninstall_package)
    ...
    this.resetInstructions = M.Modal.getInstance(
      ...
      const typeBasedActions = {
        ...
        '.obb': function(filepath, bind) {
          if (
            path
              .basename(filepath)
              .match(/main.[0-9]{1,}.[a-z]{1,}.[A-z]{1,}.[A-z]{1,}.obb/)
          ) {
            return bind.setup
              .installLocalObb(filepath)
              .then(() => bind.toggleLoader(false));
          } else {
        ...
        '.zip': function(filepath, bind) {
          return bind.setup.installLocalZip(filepath, false, () =>
            bind.toggleLoader(false)
          );
        }
      };
      let installableFiles = Object.keys(e.dataTransfer.files).filter(
        (val, index) => {
          return Object.keys(typeBasedActions).includes(
            path.extname(e.dataTransfer.files[index].name)
          );
        }
      );
  ...
  toggleLoader(show) {
    document.querySelector(
      '.spinner'
    ).style.display = this.spinner_background.style.display = show
      ? 'block'
      : 'none';
  }
  ...
  openSyncSelected() {
    this.patch_beatsaber.style.display = 'none';
    this.getMyBackups().then(async files => {
      this.patch_beatsaber.style.display = files.length
        ? 'inline-block'
        : 'none';
      this.patch_make_data_backup.style.display = files.length
        ? 'none'
        : 'inline-block';
      let questSaberPatchContainer = document.getElementById(
        'questSaberPatchContainer'
  ...
  openBackups() {
    this.getMyBackups().then(async files => {
      ...
      let dataBackupsContainer = document.getElementById(
        'dataBackupsContainer'
      );
      backupsContainer.innerHTML = files.length
        ? ''
        : '<br><h5>&nbsp;&nbsp;&nbsp;No Backups made!! Make a backup now!</h5>';
      ...
      files.forEach(f => {
        ...
        child.querySelector('.restore-backup').addEventListener('click', () => {
          ...
          this.toggleLoader(true);
          this.setup
            .installLocalApk(path.join(appData, 'bsaber-backups', f))
            .then(() => {
    ...
    this.getMyDataBackups().then(async dirs => {
      let dataBackupsContainer = document.getElementById(
        'dataBackupsContainerInner'
      );
      dirs.forEach(f => {
        let child = this.backupTemplate.content.cloneNode(true);
        child.querySelector('.backup-name').innerText = f;
        child.querySelector('.restore-backup').addEventListener('click', () => {
          this.spinner_loading_message.innerText =
            'Restoring Data Backup, Please wait...';
          this.backupsModal.close();
          this.toggleLoader(true);
          this.bsaber.restoreDataBackup(f).then(() => {
  ...
  async showInstalledPackage(child, app, appPackage) {
    ...
    if (~this.setup.devicePackages.indexOf(app.packageName)) {
      ...
      child.querySelector('.app-meta').innerHTML +=
        '<br>' +
        (hasUpdate ? 'Update Available<br>' : '') +
        'Installed version: ' +
        installedVersion +
        '<br>';
    } else {
  ...
  async openAppScreen(app, appPackage) {
    ...
    child.querySelector('.summary').innerHTML =
      this.getAppSummary(app) +
      '<br><br>' +
      this.getLongMetaData(app) +
      '<br><br>';
    child
      .querySelector('.uninstall-apk')
      .addEventListener('click', () =>
        this.setup
    ...
    appPackage.forEach((p, i) => {
      ...
      versionChild.querySelector('.version-name').innerText =
        'V. ' + p.versionName;
      versionChild
        .querySelector('.install-apk')
        .addEventListener('click', () => {
          this.toggleLoader(true);
          this.spinner_loading_message.innerText = 'Installing APK...';
          ...
          this.setup
            .installApk(
              p.apkName.substr(0, 7) === 'http://' ||
                p.apkName.substr(0, 8) === 'https://'
                ? p.apkName
                : this.current_data.url + p.apkName
            )
            .then(() => {
              this.toggleLoader(false);
              this.showStatus('APK installed!!');
      ...
      versionChild.querySelector('.permissions').innerHTML = (
        p['uses-permission'] || []
      )
        .map(permission => {
          return typeof permission === 'string'
            ? permission
            : permission.filter(p => p).join('<br>');
        })
        .join('<br>');
  ...
  async openPackageList() {
    ...
    if (!this.setup.status === 'connected') {
      this.container.innerHTML =
        '<h4 class="grey-text">No device connected...</h4>';
    } else {
      this.setup.devicePackages
        .filter(
          p =>
            !(
              p.startsWith('com.oculus') ||
              p.startsWith('com.android') ||
              p.startsWith('com.qualcomm') ||
              p.startsWith('android.ext') ||
              p === 'android' ||
              p === 'oculus.platform'
            )
        )
        .forEach((p, i) => {
          ...
          if (i % 2 === 0) {
            [].slice
              .call(child.querySelectorAll('.package-height'))
              .forEach(ele => {
          ...
          child
            .querySelector('.uninstall-package')
            .addEventListener('click', () => {
              this.current_uninstall_package = p;
              document.getElementById('app-remove-name').innerText =
      ...
      fs.readdir(backupDirectory, (err, data) => {
        if (err) {
          ...
          resolve(
            (data || []).filter(
              d =>
                fs.lstatSync(path.join(backupDirectory, d)).isDirectory() &&
                fs.existsSync(path.join(backupDirectory, d, 'PlayerData.dat'))
            )
          );
        }
      });
                  ...
                  return new Promise(r => {
                    fs.readdir(songDirectory, (err, songName) => {
                      if (err) {
                        ...
                        songName = songName.filter(d =>
                          fs
                            .lstatSync(path.join(songDirectory, d))
                            .isDirectory()
                        );
                        if (!songName.length) {
                          this.showStatus(
                            'Error, song is empty: ' + path.join(songDirectory)
                          );
                          return r();
                        }
                        ...
                        fs.readdir(
                          ...
                          async (err2, fileNames) => {
                            if (err || !fileNames.length) {
                              ...
                              if (
                                !~fileNames.indexOf('cover.jpg') &&
                                !~fileNames.indexOf('cover.png')
                              ) {
                                fs.copyFileSync(
                              ...
                              let covername = ~fileNames.indexOf('cover.jpg')
                                ? 'cover.jpg'
                                : ~fileNames.indexOf('cover.png')
                                ? 'cover.png'
                                : 'cover.jpg';
                              ...
                              song.cover =
                                'file:///' +
                                path
                                  .join(songDirectory, songName, covername)
                                  .replace(/\\/g, '/');
                              songs.push(song);
                              r();
                            }
...
<span class="link" data-url="https://bsaber.com/members/elliotttate/">@elliotttate</span> for beat saber efforts.`;
    ...
    if (isBeatSaberInstalled && isQuestSaberPatch) {
      this.container.innerHTML = songs.length
        ? ''
        : '<h4>Nothing here yet...</h4>';
      songs.forEach(song => {
        let child = this.template.content.cloneNode(true);
        child.querySelector('.image').style.backgroundImage =
          'url("' + song.cover + '")';
        child.querySelector('.image').style.backgroundColor = randomColor({
          seed: song.id
        });
        child.querySelector('.card-title-one').innerHTML =
          '<i class="material-icons right">more_vert</i>' + song.name;
        child.querySelector('.card-title-two').innerHTML =
          '<i class="material-icons right">close</i>' + song.name;
        let songIdParts = song.id.split('-');
        child.querySelector('.description').innerHTML =
          'Download URL: https://beatsaver.com/storage/songs/' +
          songIdParts[0] +
          '/' +
          songIdParts[0] +
          '-' +
          songIdParts[1] +
          '.zip<br><br><a  href="#" id="delete-song' +
          song.id +
        ...
        song.enabled = true;
        syncLabel.innerHTML =
          'Off<input type="checkbox" id="syncSong' +
          song.id +
          '" checked="checked"><span class="lever"></span>On';
        child
          .querySelector('.install-apk')
          .parentElement.appendChild(syncSwitch);
        let switchEle = child.getElementById('syncSong' + song.id);
        switchEle.addEventListener('change', () => {
          song.enabled = switchEle.checked;
        });
        child
          .querySelector('#delete-song' + song.id)
          .addEventListener('click', () => {
            this.bsaber.deleteFolderRecursive(
  ...
  openDeviceSettings() {
      ...
      ele.addEventListener('click', () => {
        ...
        this.setup.adb
          .shell(
            ...
            'setprop debug.oculus.foveation.level ' + value
          )
    ...
    pasteText.addEventListener('click', () => {
      ...
      let inputCharacters = () => {
        ...
        return this.setup.adb
          .shell(this.setup.deviceSerial, 'input text "' + character + '"')
          .then(adb.util.readAll)
          .then(res => {
    ...
    document
      .getElementById('devicePasteType')
      .addEventListener('change', function() {
        devicePaste.setAttribute('type', this.checked ? 'password' : 'text');
      });
    document
      .querySelector('.open-main-app-folder')
      .addEventListener('click', () => {
    ...
    // });
    document
      .querySelector('.open-bsaber-folder')
      .addEventListener('click', () => {
        shell.openItem(path.join(appData, 'bsaber'));
      });
    document
      .querySelector('.open-bsaber-backup-folder')
      .addEventListener('click', () => {
        shell.openItem(path.join(appData, 'bsaber-backups'));
      });
    document
      .querySelector('.open-bsaber-data-backup-folder')
      .addEventListener('click', () => {
        shell.openItem(path.join(appData, 'bsaber-data-backups'));
      });
    document
      .querySelector('.open-bsaber-patch-folder')
      .addEventListener('click', () => {
  ...
  openSetupScreen() {
    ...
    this.install_launcher_loader = document.querySelector(
      '.install-launcher-loading'
    );
    this.install_launcher_container = document.querySelector(
      '.install-launcher-container'
    );

    if (
      ~this.setup.devicePackages.indexOf('com.openappstore.wrapper') &&
      ~this.setup.devicePackages.indexOf('com.openappstore.launcher')
    ) {
      this.setup
        .uninstallApk('com.openappstore.wrapper')
        .then(() => this.setup.uninstallApk('com.openappstore.launcher'));
    }
    if (
      ~this.setup.devicePackages.indexOf('com.sidequest.wrapper') &&
      ~this.setup.devicePackages.indexOf('com.sidequest.launcher')
    ) {
      this.install_launcher_container.innerHTML =
        '<span class="chip">SideQuest Launcher Already Installed!</span>';
    } else {
    ...
    this.install_expanse_loader = document.querySelector(
      '.install-expanse-loading'
    );
    this.install_expanse_container = document.querySelector(
      '.install-expanse-container'
    );
    if (~this.setup.devicePackages.indexOf('app.theexpanse.app')) {
      this.install_expanse_container.innerHTML =
        '<span class="chip">The Expanse Already Installed!</span>';
    } else {
  ...
  searchFilter() {
    this.container.innerHTML = '';
    this.current_data.body.apps
      .filter(d => {
        let filter_value = this.filter_select.options[
          this.filter_select.selectedIndex
        ].value;
        let search_value = this.search_box.value;
        let is_filter = filter_value === d.category || filter_value === 'All';
        return (
          (is_filter && !search_value) ||
          (is_filter &&
            (~d.name.toLowerCase().indexOf(search_value.toLowerCase()) ||
              ~(d.summary || '')
                .toLowerCase()
                .indexOf(search_value.toLowerCase())))
        );
      })
      .forEach(d => {
        let child = this.template.content.cloneNode(true);
        ...
        child.querySelector('.card-title-one').innerHTML =
          '<i class="material-icons right">more_vert</i>' + d.name;
        child.querySelector('.card-title-two').innerHTML =
          '<i class="material-icons right">close</i>' + d.name;
